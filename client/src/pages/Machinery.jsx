...
export default function Machinery() {
  ...
  const [form, setForm] = useState({ site_id: '', machine_name: '', owner_vendor: '', hours_used: '', rate: '', expense_date: '' });
  ...
  const loadItems = () => {
    const url = siteFilter ? `/api/machinery?site_id=${siteFilter}` : '/api/machinery';
    api.get(url).then(setItems).catch(console.error);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    const total = (Number(form.hours_used) || 0) * (Number(form.rate) || 0);
    await api.post('/api/machinery', { ...form, total_cost: total });
    setForm({ site_id: '', machine_name: '', owner_vendor: '', hours_used: '', rate: '', expense_date: '' });
    setShowForm(false);
    loadItems();
  };

  const handleDelete = async (id) => {
    if (confirm('Delete?')) { await api.del(`/api/machinery/${id}`); loadItems(); }
  };

  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h1 className="text-2xl font-bold">🚜 Machinery Expenses</h1>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-6 mb-6">
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <input required placeholder="Machine Name *" value={form.machine_name} onChange={e => setForm({ ...form, machine_name: e.target.value })} className="border rounded-lg px-3 py-2" />
            <input placeholder="Owner/Vendor" value={form.owner_vendor} onChange={e => setForm({ ...form, owner_vendor: e.target.value })} className="border rounded-lg px-3 py-2" />
            <input type="number" placeholder="Hours Used" value={form.hours_used} onChange={e => setForm({ ...form, hours_used: e.target.value })} className="border rounded-lg px-3 py-2" />
            <input type="number" placeholder="Rate (₹/hr)" value={form.rate} onChange={e => setForm({ ...form, rate: e.target.value })} className="border rounded-lg px-3 py-2" />
          ...
          <tbody>
            {items.map(item => (
              <tr key={item.id} className="border-t hover:bg-gray-50">
                <td className="p-3 font-medium">{item.machine_name}</td>
                <td className="p-3">{item.owner_vendor}</td>
                <td className="p-3 text-right">{item.hours_used}</td>
